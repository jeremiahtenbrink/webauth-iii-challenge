...
/**
 * @api {get} /api/users/me Get me
 * @apiVersion 1.0.0
 * @apiName GetMe
 * @apiGroup Users
 * @apiPermission Users
 *
 * @apiExample Get me example:
 ...
 *
 */
usersRouter.get( '/me', restricted, async ( req: Request, res: Response ) => {
    try {
        
        const user: IUser = await Users.getUserById( req.body.decodedToken.subject );
        if ( user ) {
            let usersDisplay = {
                id: user.id,
                email: user.email,
